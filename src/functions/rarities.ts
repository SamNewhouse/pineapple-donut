...
export function assignSessionChances(
  rarities: Rarity[],
): Array<{ id: string; name: string; color: string; chance: number }> {
...
export function pickWeightedRarityId(sessionTiers: Array<{ id: string; chance: number }>): string {
  ...
  // In fallback, return the last tier's id
